...
function NumberInlineEditGenerateSelectors(fieldId, fieldName, config) {
...
function NumberInlineEditPreEnableCallback(fieldId, fieldName, config) {
	var selectors = NumberInlineEditGenerateSelectors(fieldId, fieldName, config);
	$(selectors.viewWrapper).hide();
	$(selectors.editWrapper).show();
	$(selectors.editWrapper + " .form-control").focus();
}

function NumberInlineEditPreDisableCallback(fieldId, fieldName, config) {
	var selectors = NumberInlineEditGenerateSelectors(fieldId, fieldName, config);
...
function NumberInlineEditInit(fieldId, fieldName, config) {
	...
	$(selectors.viewWrapper + " .action .btn").on("click", function (event) {
		...
		NumberInlineEditPreEnableCallback(fieldId, fieldName, config);
	});
	...
	$(selectors.editWrapper + " .cancel").on("click", function (event) {
		event.stopPropagation();
		event.preventDefault();
		NumberInlineEditPreDisableCallback(fieldId, fieldName, config);
	});
	//Save inline changes
	$(selectors.editWrapper + " .save").on("click", function (event) {
		...
		$.ajax({
			...
			success: function (response) {
				if (response.success) {
					NumberInlineEditInitSuccessCallback(response, fieldId, fieldName, config);
				}
				else {
					NumberInlineEditInitErrorCallback(response, fieldId, fieldName, config);
				}
			},
...
function NumberInlineEditInitSuccessCallback(response, fieldId, fieldName, config) {
	var selectors = NumberInlineEditGenerateSelectors(fieldId, fieldName, config);
...
function NumberInlineEditInitErrorCallback(response, fieldId, fieldName, config) {
	var selectors = NumberInlineEditGenerateSelectors(fieldId, fieldName, config);
