function PercentInlineEditGenerateSelectors(fieldId, fieldName, config) {
...
function SetHiddenPercent(fieldId, fieldName, config) {
...
function PercentInlineEditPreEnableCallback(fieldId, fieldName, config) {
	var selectors = PercentInlineEditGenerateSelectors(fieldId, fieldName, config);
	$(selectors.fakeInput).on("change paste keyup", function () {
		SetHiddenPercent(fieldId, fieldName, config);
	});
...
function PercentInlineEditPreDisableCallback(fieldId, fieldName, config) {
	var selectors = PercentInlineEditGenerateSelectors(fieldId, fieldName, config);
...
function PercentInlineEditInit(fieldId, fieldName, config) {
	...
	$(selectors.editWrapper + " .cancel").on("click", function (event) {
		event.stopPropagation();
		event.preventDefault();
		PercentInlineEditPreDisableCallback(fieldId, fieldName, config);
	});
	//Save inline changes
	$(selectors.editWrapper + " .save").on("click", function (event) {
		...
		$.ajax({
			...
			success: function (response) {
				if (response.success) {
					PercentInlineEditInitSuccessCallback(response, fieldId, fieldName, config);
				}
				else {
					PercentInlineEditInitErrorCallback(response, fieldId, fieldName, config);
				}
			},
...
function PercentInlineEditInitErrorCallback(response, fieldId, fieldName, config) {
	var selectors = PercentInlineEditGenerateSelectors(fieldId, fieldName, config);
