...
{
	[HtmlTargetElement("wv-field-phone")]
	[RestrictChildren("wv-field-prepend", "wv-field-append")]
	public class WvFieldPhone : WvFieldBase
	{
		...
		public override async Task ProcessAsync(TagHelperContext context, TagHelperOutput output)
		{
			...
			if (Mode == WvFieldRenderMode.Form)
			{
				...
				inputElCssClassList.Add("form-control erp-phone");
				...
				if (Access == WvFieldAccess.Full || Access == WvFieldAccess.FullAndCreate)
				{
					...
					if (AutoComplete != null)
					{
						...
						{
							inputEl.Attributes.Add("autocomplete", "tel");
						}
						else
						{
			...
			else if (Mode == WvFieldRenderMode.Display)
			{
				...
				if (!String.IsNullOrWhiteSpace(Value))
				{
					...
					divEl.AddCssClass("form-control-plaintext erp-phone");
			...
			else if (Mode == WvFieldRenderMode.InlineEdit)
			{
				if (Access == WvFieldAccess.Full || Access == WvFieldAccess.FullAndCreate)
				{
					#region << View Wrapper >>
					{
						...
						viewFormControlEl.AddCssClass("form-control erp-phone");
					...
					#region << Edit Wrapper>>
					{
						...
						editInputEl.AddCssClass("form-control erp-phone");
						if (Required)
						{
					...
					if (ViewContext.HttpContext.Items.ContainsKey(typeof(WvFieldPhone) + "-inline-edit"))
					{
						var tagHelperContext = (WvTagHelperContext)ViewContext.HttpContext.Items[typeof(WvFieldPhone) + "-inline-edit"];
						tagHelperInitialized = tagHelperContext.Initialized;
					}
					if (!tagHelperInitialized)
					{
						var scriptContent = WvHelpers.GetEmbeddedTextResource("inline-edit.js", "WebVella.TagHelpers.TagHelpers.WvFieldPhone", "WebVella.TagHelpers");
						var scriptEl = new TagBuilder("script");
						scriptEl.Attributes.Add("type", "text/javascript");
						scriptEl.InnerHtml.AppendHtml(scriptContent);
						output.PostContent.AppendHtml(scriptEl);

						ViewContext.HttpContext.Items[typeof(WvFieldPhone) + "-inline-edit"] = new WvTagHelperContext()
						{
							Initialized = true
						};
					...
					var scriptTemplate = @"
						$(function(){
							PhoneInlineEditInit(""{{FieldId}}"",""{{Name}}"",{{ConfigJson}});
						});";
					...
					var fieldConfig = new WvFieldPhoneConfig()
					{
						ApiUrl = ApiUrl,
				...
				else if (Access == WvFieldAccess.ReadOnly)
				{
					...
					//Control
					var inputEl = new TagBuilder("input");
					inputEl.AddCssClass("form-control erp-phone");
					inputEl.Attributes.Add("type", "text");
					inputEl.Attributes.Add("value", (Value ?? "").ToString());
					inputEl.Attributes.Add("readonly", null);
