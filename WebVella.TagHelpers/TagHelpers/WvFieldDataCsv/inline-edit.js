...
function DataCsvInlineEditGenerateSelectors(fieldId, fieldName, delimiterFieldName, hasHeaderFieldName,hasHeaderColumnFieldName, config) {
    ...
    $selectors.modalInputHasHeaderColumnEl = $selectors.modal.find("input[name='" + hasHeaderColumnFieldName + "']");
    $selectors.modalInputHasHeaderColumnFakeEl = $selectors.modal.find("#input-hasheader-column-fake-" + fieldId);
...
function DataCsvInlineEditInit(fieldId, fieldName, delimiterFieldName, hasHeaderFieldName,hasHeaderColumnFieldName, config) {
    ...
    $selectors.modalInputHasHeaderColumnFakeEl.on('change', function () {
        if ($(this).prop('checked')) {
            $($selectors.modalInputHasHeaderColumnEl).val("true");
            $($selectors.modalInputHasHeaderColumnEl).trigger("change");
        }
        else {
            $($selectors.modalInputHasHeaderColumnEl).val("false");
            $($selectors.modalInputHasHeaderColumnEl).trigger("change");
        }
    });
    ...
    $selectors.modalSubmitBtn.click(function (event) {
        ...
        if ($selectors.modalInputHasHeaderColumnEl.val() === "false") {
        ...
        payload[hasHeaderColumnFieldName] = hasHeaderColumn;
