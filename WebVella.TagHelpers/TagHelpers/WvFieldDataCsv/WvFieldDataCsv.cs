...
namespace WebVella.Erp.Web.TagHelpers
{
	...
	public class WvFieldDataCsv : WvFieldBase
	{
		...
		public WvCsvDelimiterType DelimiterValue { get; set; } = WvCsvDelimiterType.COMMA;
		...
		[HtmlAttributeName("preview-api-url")]
		...
		public override void Process(TagHelperContext context, TagHelperOutput output)
		{
			...
			if (String.IsNullOrWhiteSpace(HasHeaderColumnFieldName))
			{
			...
			if (Mode == WvFieldRenderMode.Form)
			{
				...
				//hasHeader
				{
					...
					var textHeadHeader = "header row";
					if (Lang == "bg")
					{
						textHeadHeader = "заглавен ред";
					}
				...
				//hasHeaderColumn
				{
					var hasHeaderColumnWrapper = new TagBuilder("div");
					hasHeaderColumnWrapper.AddCssClass("form-check form-check-inline mr-5");
					...
					{
						hasHeaderColumnCheck.Attributes.Add("value", "false");
					}
					hasHeaderColumnWrapper.InnerHtml.AppendHtml(hasHeaderColumnCheck);

					var inputColumnFake = new TagBuilder("input");
					inputColumnFake.AddCssClass("form-check-input");
					inputColumnFake.Attributes.Add("id", $"input-hasheader-column-fake-{FieldId}");
					...
					hasHeaderColumnWrapper.InnerHtml.AppendHtml(inputColumnFake);

					var labelColumnWrapper = new TagBuilder("label");
					labelColumnWrapper.AddCssClass("form-check-label");
					labelColumnWrapper.Attributes.Add("for", "input-hasheader-column-fake-" + FieldId);
					var textHeadColumnHeader = "header column";
					if (Lang == "bg")
					{
						textHeadColumnHeader = "заглавна колона";
					}
					labelColumnWrapper.InnerHtml.AppendHtml(textHeadColumnHeader);

					hasHeaderColumnWrapper.InnerHtml.AppendHtml(labelColumnWrapper);
					auxInputWrapper.InnerHtml.AppendHtml(hasHeaderColumnWrapper);
				...
				if (DelimiterValue == WvCsvDelimiterType.COMMA)
				{
				...
				{
					var scriptContent = WvHelpers.GetEmbeddedTextResource(fileName, "WebVella.TagHelpers.TagHelpers.WvFieldDataCsv", "WebVella.TagHelpers");
				...
				var scriptTemplate = @"
						$(function(){
							DataCsvFormInit(""{{FieldId}}"",""{{Name}}"",""{{DelimiterFieldName}}"",""{{HasHeaderFieldName}}"",""{{HasHeaderColumnFieldName}}"",""{{Lang}}"",{{ConfigJson}});
						});";
				...
				scriptTemplate = scriptTemplate.Replace("{{HasHeaderColumnFieldName}}", HasHeaderColumnFieldName);
				scriptTemplate = scriptTemplate.Replace("{{Lang}}", Lang);

				var fieldConfig = new WvFieldDataCsvConfig()
				{
					ApiUrl = ApiUrl,
			...
			else if (Mode == WvFieldRenderMode.InlineEdit)
			{
				...
				if (Access == WvFieldAccess.Full || Access == WvFieldAccess.FullAndCreate)
				{
					...
					//hasHeaderColumn
					{
						...
						labelColumnWrapper.AddCssClass("form-check-label");
						labelColumnWrapper.Attributes.Add("for", "input-hasheader-column-fake-" + FieldId);
						var textHeadHeader = "header column";
						if (Lang == "bg")
						{
							textHeadHeader = "заглавна колона";
						}
						labelColumnWrapper.InnerHtml.AppendHtml(textHeadHeader);
				...
				var scriptTemplate = @"
						$(function(){
							DataCsvInlineEditInit(""{{FieldId}}"",""{{Name}}"",""{{DelimiterFieldName}}"",""{{HasHeaderFieldName}}"",""{{HasHeaderColumnFieldName}}"",{{ConfigJson}});
						});";
