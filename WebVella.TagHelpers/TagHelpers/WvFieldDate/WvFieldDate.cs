	...
	public class WvFieldDate : WvFieldBase
	{
		...
		public override async Task ProcessAsync(TagHelperContext context, TagHelperOutput output)
		{
			...
			if (Mode == WvFieldRenderMode.Form)
			{
				...
				if (Value != null)
				{
					inputReadOnlyValue = (Value as DateTime? ?? (DateTime?)DateTime.Now).ConvertToTZDate(TimezoneName).Value.ToString("dd MMM yyyy", Culture);
				...
				if (Access == WvFieldAccess.Full || Access == WvFieldAccess.FullAndCreate)
				{
					...
					{
						var scriptContent = WvHelpers.GetEmbeddedTextResource("form.js", "WebVella.TagHelpers.TagHelpers.WvFieldDate");
			...
			else if (Mode == WvFieldRenderMode.Display)
			{
				if (Value != null)
				{
					var inputReadOnlyValue = (Value as DateTime? ?? (DateTime?)DateTime.Now).ConvertToTZDate(TimezoneName).Value.ToString("dd MMM yyyy", Culture);
			...
			else if (Mode == WvFieldRenderMode.InlineEdit)
			{
				if (Access == WvFieldAccess.Full || Access == WvFieldAccess.FullAndCreate)
				{
					...
					if (Value != null)
					{
						readOnlyInputValue = (Value as DateTime? ?? (DateTime?)DateTime.Now).ConvertToTZDate(TimezoneName).Value.ToString("dd MMM yyyy", Culture);
					...
					if (!tagHelperInitialized)
					{
						var scriptContent = WvHelpers.GetEmbeddedTextResource("inline-edit.js", "WebVella.TagHelpers.TagHelpers.WvFieldDate");
						var scriptEl = new TagBuilder("script");
						scriptEl.Attributes.Add("type", "text/javascript");
						scriptEl.InnerHtml.AppendHtml(jsCompressor.Compress(scriptContent));
						output.PostContent.AppendHtml(scriptEl);
