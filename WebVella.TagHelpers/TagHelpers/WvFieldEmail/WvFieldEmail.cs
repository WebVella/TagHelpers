		...
		public override async Task ProcessAsync(TagHelperContext context, TagHelperOutput output)
		{
			...
			else if (Mode == WvFieldRenderMode.InlineEdit)
			{
				if (Access == WvFieldAccess.Full || Access == WvFieldAccess.FullAndCreate)
				{
					...
					{
						var scriptContent = WvHelpers.GetEmbeddedTextResource("inline-edit.js", "WebVella.TagHelpers.TagHelpers.WvFieldEmail", "WebVella.TagHelpers");
					...
					var scriptTemplate = @"
						$(function(){
							EmailInlineEditInit(""{{FieldId}}"",""{{Name}}"",{{ConfigJson}});
						});";
