...
function HtmlInlineEditGenerateSelectors(fieldId, fieldName,config) {
...
function HtmlInlineEditPreEnableCallback(fieldId, fieldName,config) {
...
function HtmlInlineEditPreDisableCallback(fieldId, fieldName,config) {
	var selectors = HtmlInlineEditGenerateSelectors(fieldId, fieldName,config);
...
function HtmlInlineEditInit(fieldId, fieldName,config) {
	...
	$(selectors.editWrapper + " .save").on("click", function (event) {
		event.stopPropagation();
		event.preventDefault();
		var inputValue = $('#input-' + fieldId).val();
		...
		$.ajax({
			...
			success: function (response) {
				if (response.success) {
					HtmlInlineEditInitSuccessCallback(response, fieldId, fieldName,config);
				}
				else {
					HtmlInlineEditInitErrorCallback(response, fieldId, fieldName,config);
				}
			},
...
function HtmlInlineEditInitErrorCallback(response, fieldId, fieldName,config) {
	var selectors = HtmlInlineEditGenerateSelectors(fieldId, fieldName,config);
