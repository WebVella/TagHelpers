...
{
	[HtmlTargetElement("wv-field-time")]
	[RestrictChildren("wv-field-prepend", "wv-field-append")]
	public class WvFieldTime : WvFieldBase
	{
		...
		public override async Task ProcessAsync(TagHelperContext context, TagHelperOutput output)
		{
			...
			if (Mode == WvFieldRenderMode.Form)
			{
				...
				if (Access == WvFieldAccess.Full || Access == WvFieldAccess.FullAndCreate)
				{
					...
					if (ViewContext.HttpContext.Items.ContainsKey(typeof(WvFieldTime) + "-form"))
					{
					...
					{
						var scriptContent = WvHelpers.GetEmbeddedTextResource("form.js", "WebVella.TagHelpers.TagHelpers.WvFieldTime");
					...
					var scriptTemplate = @"
						$(function(){
							InitFlatPickrTime(""#input-{{FieldId}}"");
						});";
			...
			else if (Mode == WvFieldRenderMode.InlineEdit)
			{
				if (Access == WvFieldAccess.Full || Access == WvFieldAccess.FullAndCreate)
				{
					...
					#region << Init Scripts >>
					var tagHelperInitialized = false;
					if (ViewContext.HttpContext.Items.ContainsKey(typeof(WvFieldTime) + "-inline-edit"))
					{
						var tagHelperContext = (WvTagHelperContext)ViewContext.HttpContext.Items[typeof(WvFieldTime) + "-inline-edit"];
						tagHelperInitialized = tagHelperContext.Initialized;
					}
					if (!tagHelperInitialized)
					{
						var scriptContent = WvHelpers.GetEmbeddedTextResource("inline-edit.js", "WebVella.TagHelpers.TagHelpers.WvFieldTime");
						var scriptEl = new TagBuilder("script");
						scriptEl.Attributes.Add("type", "text/javascript");
						scriptEl.InnerHtml.AppendHtml(jsCompressor.Compress(scriptContent));
						output.PostContent.AppendHtml(scriptEl);
					...
					var scriptTemplate = @"
						$(function(){
							TimeInlineEditInit(""{{FieldId}}"",""{{Name}}"",{{ConfigJson}});
						});";
					scriptTemplate = scriptTemplate.Replace("{{FieldId}}", (FieldId ?? null).ToString());
