...
function TimeInlineEditGenerateSelectors(fieldId, fieldName, config) {
...
function TimeInlineEditPreEnableCallback(fieldId, fieldName, config) {
...
function TimeInlineEditPreDisableCallback(fieldId, fieldName, config) {
	var selectors = TimeInlineEditGenerateSelectors(fieldId, fieldName, config);
...
function TimeInlineEditInit(fieldId, fieldName, config) {
	...
	$(selectors.editWrapper + " .cancel").on("click", function (event) {
		event.stopPropagation();
		event.preventDefault();
		TimeInlineEditPreDisableCallback(fieldId, fieldName, config);
	});
	//Save inline changes
	$(selectors.editWrapper + " .save").on("click", function (event) {
		...
		$.ajax({
			...
			success: function (response) {
				if (response.success) {
					TimeInlineEditInitSuccessCallback(response, fieldId, fieldName, config);
				}
				else {
					TimeInlineEditInitErrorCallback(response, fieldId, fieldName, config);
				}
			},
...
function TimeInlineEditInitErrorCallback(response, fieldId, fieldName, config) {
	var selectors = TimeInlineEditGenerateSelectors(fieldId, fieldName, config);
