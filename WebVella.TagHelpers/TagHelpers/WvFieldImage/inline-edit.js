...
function ImageInlineEditGenerateSelectors(fieldId, fieldName,config) {
...
function ImageInlineEditInit(fieldId, fieldName,config) {
	...
	$(selectors.removeValueEl).first().on('click', function (e) {
		...
		if (result) {
			...
			ImageInlineEditUploadSuccessCallback(response, fieldId, fieldName,config)
		}
	});
			...
			if (window.FormData !== undefined) {
				...
				$.ajax({
					...
					success: function (result) {
						ImageInlineEditUploadSuccessCallback(result, fieldId, fieldName,config)
					},
...
function ImageInlineEditUploadSuccessCallback(response, fieldId, fieldName,config) {
	var selectors = ImageInlineEditGenerateSelectors(fieldId, fieldName,config);
		...
		$.ajax({
			...
			success: function (result) {
				if (result.success) {
					...
					ImageInlineEditInitSuccessCallback(result, fieldId, fieldName,config);
				}
				else {
					ImageInlineEditInitErrorCallback(result, fieldId, fieldName,config);
				}
			},
...
function ImageInlineEditInitErrorCallback(response, fieldId, fieldName,config) {
	var selectors = ImageInlineEditGenerateSelectors(fieldId, fieldName,config);
	$(selectors.editWrapperText).first().html("<i class='fa fa-exclamation-circle go-red'></i> Error");
