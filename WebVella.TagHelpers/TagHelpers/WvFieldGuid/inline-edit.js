...
function GuidInlineEditGenerateSelectors(fieldId, fieldName, config) {
...
function GuidInlineEditPreEnableCallback(fieldId, fieldName, config) {
	var selectors = GuidInlineEditGenerateSelectors(fieldId, fieldName, config);
	$(selectors.viewWrapper).hide();
	$(selectors.editWrapper).show();
	$(selectors.editWrapper + " .form-control").focus();
}

function GuidInlineEditPreDisableCallback(fieldId, fieldName, config) {
	var selectors = GuidInlineEditGenerateSelectors(fieldId, fieldName, config);
...
function GuidInlineEditInit(fieldId, fieldName, config) {
	...
	$(selectors.viewWrapper + " .action .btn").on("click", function (event) {
		...
		GuidInlineEditPreEnableCallback(fieldId, fieldName, config);
	});
	...
	$(selectors.editWrapper + " .cancel").on("click", function (event) {
		event.stopPropagation();
		event.preventDefault();
		GuidInlineEditPreDisableCallback(fieldId, fieldName, config);
	});
	//Save inline changes
	$(selectors.editWrapper + " .save").on("click", function (event) {
		...
		$.ajax({
			...
			success: function (response) {
				if (response.success) {
					GuidInlineEditInitSuccessCallback(response, fieldId, fieldName, config);
				}
				else {
					GuidInlineEditInitErrorCallback(response, fieldId, fieldName, config);
				}
			},
...
function GuidInlineEditInitSuccessCallback(response, fieldId, fieldName, config) {
	var selectors = GuidInlineEditGenerateSelectors(fieldId, fieldName, config);
...
function GuidInlineEditInitErrorCallback(response, fieldId, fieldName, config) {
	var selectors = GuidInlineEditGenerateSelectors(fieldId, fieldName, config);
